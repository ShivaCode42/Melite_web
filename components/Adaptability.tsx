...
export const Adaptability: React.FC = () => {
  ...
  // Helper component to render module content to avoid duplication
  const RenderModuleContent = ({ tabName }: { tabName: string }) => {
    const content = MODULE_CONTENT[tabName];
  ...
  return (
    <section id="produkty" className="py-12 md:py-20 bg-dot-grid">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Label and Layout Switcher */}
        <ScrollReveal delay={200}>
          <div className="text-center mb-10">
            <span className="text-gray-400 font-bold text-lg uppercase tracking-widest inline-block border-b-2 border-blue-50 pb-2">
              Moduly
            </span>
          </div>
        </ScrollReveal>

        {/* Desktop View: Tabbed Interface (Visible from md up) */}
        <div className="hidden md:block">
          ...
          <div key={activeTab}>
            <ScrollReveal delay={400}>
              <RenderModuleContent tabName={activeTab} />
            </ScrollReveal>
          </div>
        </div>

        {/* Mobile View: Vertical Stacked Interface (Visible below md) */}
        <div className="md:hidden space-y-20">
          {ADAPTABILITY_TABS.map((tab, idx) => (
            <div key={tab}>
              <ScrollReveal delay={idx * 100}>
                ...
                <RenderModuleContent tabName={tab} />
              </ScrollReveal>
            </div>
          ))}
        </div>
